...
## 18、SQL 注入
   （1）把 SQL 命令插入到 Web 表单提交或页面请求的查询字符串，导致服务器执行恶意的 SQL 命令。
...
## 19、DDOS 攻击,全称是 Distributed Denial of Service，翻译成中文就是分布式拒绝服务
      （1）DDOS 攻击利用目标系统网络服务功能缺陷或者直接消耗其系统资源，使得该目标系统无法提供正常的服务。
      
      （2）DDoS 攻击通过大量合法的请求占用大量网络资源，以达到瘫痪网络的目的。
      
      （3）防御：
              高防服务器，但硬件贵
              黑名单，影响业务，错杀
              DDoS 清洗，监控异常数据，实时清除异常请求。
              CDN分流，隐藏真实IP，减轻服务压力

## 20、SYN Flood，是拒绝服务（DDoS）攻击的一种。
     （1）SYN Flood 就是用户向服务器发送报文后突然死机或掉线，那么服务器在发出应答报文后就无法收到客户端的确认报文（第三次握手无法完成），使得被攻击方资源耗尽（CPU满负荷或内存不足）。
...
## 23、http
 ...
 ## 26、javascript继承方式
   （1）原型链继承：将父类实例作为子类原型
        ...
        缺点：
        1）不可以向父类构造函数传参数；
        2）子类实例共享了父类构造函数的引用属性，比如arr属性，但对整个arr重新赋值（相当于指向了另外的内存地址）并不会影响到其他子类。
        
   （2）借用构造函数：不用原型，借用父类的构造函数来增强子类实例，即复制父类的实例属性给子类。
      
      解决原型链继承的缺点，但是父类方法不能复用，更重要的是原型上的方法属性也不能用，因为压根就没用的原型。
      
   （3）组合继承：通过调用父类构造函数，继承父类的属性并保留传参的优点；然后通过将父类实例作为子类原型，实现函数复用。Child.prototype = new Parent()
   
         解决了上面两种方式的缺点，但是由于调用了2次父类的构造方法，会存在一份多余的父类实例属性，另外还需要更改子类构造函数的指向。
         
   （4）组合继承优化：在3的基础上，不再将父类实例作为子类原型，而是改为父类的原型作为子类的原型。Child.prototype = Parent.prototype
   
         解决了3中的问题，只实例化一次父类构造函数。
         缺点是：修复子类构造函数的指向后，父类实例的构造函数指向也会跟着变成了子类的构造函数。
 ...
 ## 28、为什么建立连接协议是三次握手，而关闭连接却是四次握手呢？

   （1）这是因为服务端的LISTEN状态下的SOCKET当收到SYN报文的连接请求后，它可以把ACK和SYN(ACK起应答作用，而SYN起同步作用)放在一个报文里来发送。但关闭连接时，当收到对方的FIN报文通知时，它仅仅表示对方没有数据发送给你了；但未必你所有的数据都全部发送给对方了，所以你可能未必会马上会关闭SOCKET,也即你可能还需要发送一些数据给对方之后，再发送FIN报文给对方来表示你同意现在可以关闭连接了，所以它这里的ACK报文和FIN报文多数情况下都是分开发送的。
...
## 44、箭头函数特点
   ...
   箭头函数会默认帮我们绑定外层this的值，所以在箭头函数中this的值和外层的this是一样的。外层必须能形成一个作用域，函数可以，对象不可以；
